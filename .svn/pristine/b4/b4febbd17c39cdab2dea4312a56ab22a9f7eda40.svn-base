 ...
 * 
 * @author kinsly 2013-8-19 下午12:43:41
 *
 */
class ZcSqlBuilder {
	public $argChar = '%';
	...
	public function buildInsertUpdate($args) {
		$table = array_shift($args);
		$datas = array_shift($args);
		...
		// 如果没有第三个参数了，说明是要整条记录更新的节奏了
		if (!isset($args[0])) {
			if (isset($datas[0]) && is_array($datas[0])) {
				throw new ZcDbException("Badly formatted insertUpdate() query -- you didn't specify the update component!");
			}
			$args[0] = $datas;
		}
		if (is_array($args[0])) {
		...
		if ($endWithArray) {
			$buildQuery = str_replace('%', $this->argChar, "INSERT INTO %b %lb VALUES %? ON DUPLICATE KEY UPDATE %?");
			return call_user_func_array(array($this, 'buildSql'), array($buildQuery, $table, $keys, $values, $update));
		} else {
			$updateStr = array_shift($update); 
			$buildQuery =  str_replace('%', $this->argChar, "INSERT INTO %b %lb VALUES %? ON DUPLICATE KEY UPDATE ") . $updateStr;
			$param = array($buildQuery, $table, $keys, $values);
			$param = array_merge($param, $update);
			return call_user_func_array(array($this, 'buildSql'), $param);
		}
	}
