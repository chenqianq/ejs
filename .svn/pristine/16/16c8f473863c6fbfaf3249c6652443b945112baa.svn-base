 ...
 * 
 * @author kinsly 2012-06-19 14:30
 *
 */
class ZcMonitorHandler {
	...
	public function monitor($errorStr, $errorType = 1, $needNotify = false) {
		...
		$isInsertAction = stripos(strtolower($errorContents),'insert into');
		$insertError = '';
		if( $isInsertAction !== false ){
			$endInsertActionPos = stripos(strtolower($errorContents),'mysql');
			$insertError = substr($errorContents,$isInsertAction,$endInsertActionPos - $isInsertAction - 3);
		}
		
		$isInsertAction = stripos(strtolower($errorContents),'update');
		if( $isInsertAction !== false ){
			$endInsertActionPos = stripos(strtolower($errorContents),'mysql');
			$insertError = substr($errorContents,$isInsertAction,$endInsertActionPos - $isInsertAction - 3);
		}
		...
		if( stripos($errorContents,'deprecated') !== false ){
		    return false;
		}
		...
		if( $insertError ){
			//$query = "INSERT INTO `innisfree_orders` (`gmt_create`, `gmt_modified`,`innisfree_orders_sql`) VALUES (now(), now(), '".$dbLink->escape($insertError)."');";
			//$dbLink->query($query);
		}
