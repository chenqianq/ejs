<?php
class HtmlTool extends ZcPermission{
	 ...
	 */
	public static function getStaticFile($filename,$onlyUrl=false) {
		$files = array ();
		$static = array ();
	...
	/**
	 * 翻译route把大字字母转换成下划线加小写字母
	 */
	public static function transLateRoute($string) {
		$translateString = '';
		for($i = 0; $i < strlen($string) ; $i++) {
			if(ord($string[$i]) >= 65 && ord($string[$i]) <= 90) {
				$translateString .= ($i != 0) ? ("_" . chr(ord($string[$i])+32)) : chr(ord($string[$i])+32);  //将大写字母转换成小写，且非首字母转换的加个下划线
			} else {
				$translateString .= $string[$i];
			}
		}
