		...
		<form class="layui-form">
			...
			<div class="layui-form-item">
				<div class="layui-input-block">
					<input type="hidden" value="<?php echo $businessInfo['business_manager_id'];?>" name="business_manager_id" id="business_manager_id">
    ...
    layui.use('form', function(){
        ...
        form.on('submit(formDemo)', function(data){
            ...
            layer.confirm('确认保存当前内容',{btn:['确定','取消']},function(){
                ...
                var business_manager_id = $('#business_manager_id').val();
                if(!business_manager_id){
                    var url = 'save_add_business';
                }else{
                    var url = 'save_edit_business?business_manager_id'+business_manager_id;
                }
                layer.closeAll();
                $.ajax({
                    url:url,
