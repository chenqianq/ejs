...
class Curl {
	 ...
	 * @return 错误返回:false 正确返回:结果内容
	 */
	public function post($url, $params = array(), $uploadFile = array(), $referer = '',$isDataFlow = false) {
		
		return $this->_request ( 'POST', $url, $params, $uploadFile, $referer , $isDataFlow );
	}
	 ...
	 * @return 错误返回:false 正确返回:结果内容
	 */
	private function _request($method, $url, $params = array(), $uploadFile = array(), $referer = '' , $isDataFlow = false) {
		...
		if ($method == 'POST') {
			...
			if( $uploadFile ){
				//var_dump($postData,($uploadFile?false:true));exit;
			}
