...
class ResizeImage{
    ...
    private $compressionUrl;                   //压缩地址

    private $compressionMinSize;
    function __construct()
    {
        $this->compressionUrl = "http://" . COMPRESSION_HOST . ":" . COMPRESSION_PORT . COMPRESSION_PATH;
        $this -> compressionMinSize = 200*1024;
    }
     ...
     */
    public function newImg($img, $wid, $hei,$scale,$small_ext="_small.",$dst_img='',$filling = true,$is_tailor=false){
        $this->config['impath'] = zc::C('thumb.impath');
...
// 		$this->config['thumb_quality'] = (C('thumb_quality') ? C('thumb_quality') : 100);
    ...
    private function tailorImg(){
        ...
        imagecopyresampled($newimg,$this->im,0,0,($this->width-$this->resize_width)/2-40,($this->height-$this->resize_height)/2,$this->resize_width,$this->resize_height,$this->resize_width,$this->resize_height);
...
//		}
    ...
    private function dst_img()
    {
        ...
        $this->relative_dstimg = str_replace(zc::C('base_path'),'',$this->dstimg);
    }
     ...
     * @return bool
     */
    public function compressionImg($imgUrl, $thumb_quality=0.7,$exportPath="",$imageSize=0)
    {
        if (empty($exportPath)) {
            $exportPath = $imgUrl;
        }
        if($imageSize <= $this -> compressionMinSize){
            return false;
        }
        $type = substr(strrchr($imgUrl, "."), 1);
        if (strtolower($type) == "png") {
            $img = imagecreatefrompng($imgUrl);
            imagejpeg($img, $exportPath, 80);
            ImageDestroy($img);
            return true;
        }
        $url = $this->compressionUrl . "?imagePath=" . urlencode($imgUrl) . "&quality=" . $thumb_quality . "&outputPath=" . urlencode($exportPath);
        $ch = curl_init();
