...
function regionInit(){
	//var area_id = 0;
	var divId = arguments[0] ? arguments[0] : 1;
	var area_id = arguments[1] ? arguments[1] : 0;
	getArea(function(){
	...
	getArea(function(){
		if(typeof(area_select) == 'object' && nc_a[area_id].length > 0){
			...
			for (i = 0; i <areas.length; i++){
				$(area_select).append("<option "+ (area_id == areas[i][0] ? 'selected' : '') +" value='" + areas[i][0] + "'>" + areas[i][1] + "</option>");
			}
		}
	});
}

function areaInitNext(area_select,area_id,currentAreaId){//初始化地区
	...
	getArea(function(){
		if(typeof(area_select) == 'object' && nc_a[area_id].length > 0){
			...
			$(area_select).append("<option>-请选择-</option>");
			for (i = 0; i <areas.length; i++){ 
				$(area_select).append("<option "+ (currentAreaId == areas[i][0] ? 'selected' : '') +" value='" + areas[i][0] + "'>" + areas[i][1] + "</option>");
			}
		}
	});
}

function areaInitNew(area_select,area_id){//初始化地区
	...
	getArea(function(){
		...
		$(area_select).change(regionChange); // select的onchange事件
		$("#region > input:button[class='edit_region']").click(regionEdit); // 编辑按钮的onclick事件
	});
}
