...
class PageSplit {
	...
	public $text_last = '末页';
	public $outer_element='<div class="pagination MT20"><ul>{0}</ul></div>';
	//替换的是页面数字
	public $no_active_page='<li><span>{0}</span></li>';
	//替换的是a链接
	...
	public function render() {
		...
		if ($page > 1) {
			$output .= $this->commonHelper->format($this->active_page,' <a  data-page="1"  class="'.$this->active_class.'" href="' . str_replace ( '{page}', 1, $this->url ) . '">' . $this->text_first . '</a> <a data-page="'.($page - 1).'" class="'.$this->active_class.'" href="' . str_replace ( '{page}', $page - 1, $this->url ) . '">' . $this->text_prev . '</a> ');
		}else{
		...
		if ($num_pages > 1) {
			...
			for($i = $start; $i <= $end; $i ++) {
				if ($page == $i) {
					...
					$output .=$this->commonHelper->format($this->active_page,' <a data-page="'.$i.'" href="' . str_replace ( '{page}', $i, $this->url ) . '">' . $i . '</a> ') ;
				}
			}
		...
		if ($page < $num_pages) {
			$output .= $this->commonHelper->format($this->active_page,' <a data-page="'.($page + 1).'"  class="'.$this->active_class.'" href="' . str_replace ( '{page}', $page + 1, $this->url ) . '">' . $this->text_next . '</a> <a data-page="'.$num_pages.'"  class="'.$this->active_class.'" href="' . str_replace ( '{page}', $num_pages, $this->url ) . '">' . $this->text_last . '</a> ');
		}else{
