...
class ZcDbRbac extends ZcRbac {
	...
	public function updatePermission(ZcPermission $permission) {
		...
		try {
			$this->db->update(TableConst::TABLE_ADMIN_PERMISSION, array (
					'admin_permission_desc' => $permission->getDesc(),
					'gmt_modify' => date('Y-m-d H:i:s',time())
			), 'admin_permission_id = %i', $permission->getZcPermissionId());
	...
	public function getPermission($permissionId) {
		...
		$permissionRow = $this->db->queryFirstRow("select * from " . TableConst::TABLE_ADMIN_PERMISSION . " where admin_permission_id = %i", $permissionId);
		return new ZcPermission($permissionRow['admin_permission_id'], $permissionRow['admin_permission_name'], $permissionRow['admin_permission_desc']);
	}
	 ...
	 * @return array
	 */
	public function getAllPermission($userId = null,$page= 1,$pageSize = 20,$rows=array()) {
        $sql = '';
        if ($userId > 0) {
            $rps = $this->getUserAllRoleIdsAndPermissionIds($userId);
            $userPermissionIds = $rps[1];
            $sql = $this->db->prepare("select * from " . TableConst::TABLE_ADMIN_PERMISSION . " where admin_permission_id in %li ORDER BY `admin_permission_id` desc", $userPermissionIds);
        } else {
            $where = "";
            $sql = "select * from " . TableConst::TABLE_ADMIN_PERMISSION . $where . " ORDER BY `admin_permission_id` desc";
        }
	...
	public function deletePermissions($permissionIds) {
		...
		try {
			$this->db->delete(TableConst::TABLE_ADMIN_PERMISSION, 'admin_permission_id in %li', $permissionIds);
			$this->db->delete(TableConst::TABLE_ADMIN_GROUP_TO_PERMISSION, 'admin_permission_id in %li', $permissionIds);
	 ...
	 * @return bool
	 */
	public function addPermission(ZcPermission $permission,$permisionType=0) {
		$ret = $this->db->insert(TableConst::TABLE_ADMIN_PERMISSION, array (
			'admin_permission_name' => $permission->getName(),
			'gmt_add' => date('Y-m-d H:i:s',time()),
            'permission_type' => $permisionType
		));
		if ($ret === false) {
			return false;
		}
     ...
     * @return array
     */
    public function getAllPermissionBySite($userId = null,$page= 1,$pageSize = 20, $site ='')
    {
        if ($userId > 0) {
            $rps = $this->getUserAllRoleIdsAndPermissionIds($userId);
            $userPermissionIds = $rps[1];
     ...
     * @return bool
     */
    public function updatePermissionModuleAndClass(ZcPermission $permission) {
        $transStatus = $this->db->startTransaction();
        $oldErrorMode = $this->db->setErrorMode(ZcDB::ERROR_MODE_EXCEPTION);

        try {
            $this->db->update(TableConst::TABLE_ADMIN_PERMISSION, array (
                'module_name' => $permission->getModuleName(),
                'module_desc' => $permission->getModuleDesc(),
                'class_name' => $permission->getClassName(),
                'class_desc' => $permission->getClassDesc(),
                'gmt_modify' => date('Y-m-d H:i:s',time())
            ), 'admin_permission_id = %i', $permission->getZcPermissionId());
     ...
     * @return bool
     */
    public function addPermissionWithModuleAndClass(ZcPermission $permission) {
        $ret = $this->db->insert(TableConst::TABLE_ADMIN_PERMISSION, array (
            'admin_permission_name' => $permission->getName(),
            'gmt_add'               => date('Y-m-d H:i:s', time()),
            'gmt_modify'               => date('Y-m-d H:i:s', time()),
