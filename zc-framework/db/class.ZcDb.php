 ...
 *
 * @author kinsly 2013-8-12 下午2:18:04
 *
 */
class ZcDb {
	...
	/**
	 * @var ZcDbDefaultListener
	 */
	private $dbDefaultListener;
	...
	public function __construct($config = ''){
		...
		if (isset($this->config['db_cache'])) {
			...
			if( strtolower($cc['cache_type']) != 'file' ){ 
			    if( !$this -> cache -> set('test-redis','test') ){ 
			        $options = [
			            'temp' => dirname(DIR_FS_DOCUMENT_ROOT) .'/logs/'
			        ];
			        
			        $this->cache = Zc::getCache($cc['biz_name'], 'file', $cc['timestamp'],$options,'',true); 
		...
		$this->dbDefaultListener = new ZcDbDefaultListener();
		$this->addDbListener($this->dbDefaultListener);
		$this->addDbListener(new YifanjieDbListener());
	}
	...
	private function innerExec($sqlType, $args) {
		try {
			...
			} else if ($sqlType == self::SQL_INSERTUPDATE) {
				$sql = $sb->buildInsertUpdate($args);
	 ...
	 * @param $tableName
	 * @param $data
	 * @return 如果是真正是insert，返回1；如果是update，返回2。
	 */
	public function insertUpdate($tableName, $data) {
		$args = func_get_args();
		return $this->innerExec(self::SQL_INSERTUPDATE, $args);
	}
	...
	private function queryHelper() {
		try {
			...
			$ret = false;
			$cacheKey = md5($sql);
			$refreshCache = $_GET['refresh_cache'];
			if ($currCacheExpire > 0 && $this->cache && !$refreshCache) {
	 ...
	 * @return boolean
	 */
	public function coverConnection($dbId,$exceptDbIdArray=array()){
		if( !$this->dbIdMapping[$dbId] ){
			return false;
		}
		...
		$coverConfig = $this->dbIdMapping[$dbId];
		foreach ( $this->dbIdMapping as $keyDbId => $dbIdMapping ){
			if( in_array($keyDbId,$exceptDbIdArray) ){
				continue;
			}
			
			if( in_array($keyDbId,array('log','black')) ){
				continue;
			}
			
			$this->dbIdMapping[$keyDbId] = $coverConfig;
		}
		return true;
	}
	...
	public function getStats() {
	    return $this->dbDefaultListener->getTotalStats();
	}
}