 ...
 * Zc 2.0的入口文件
 *
 * @author kinsly 2013-6-28 下午3:42:28
 *
 */
class Zc {
	// Zc框架文件所在的根目录
	...
	public static function initZcFrameworkAutoloader() {
		...
		self::$zcClassMapping = array(
				...
				'ZcCacheRedis' => 'cache/class.ZcCacheRedis.php',
		        ...
		        'YifanjieDbListener' => 'db/class.YifanjieDbListener.php',
				...
				'CpaZcDbRbac' => '/web/auth/class.CpaZcDbRbac.php',
	...
	public static function W($route, $data=array(), $return=false) {
		...
		$widgetClassFile = Zc::C(ZcConfigConst::DirFsLibsWidget) . $path . 'class.' . $className . '.php';
		if (file_exists($widgetClassFile)) {
			require_once($widgetClassFile);
			$widget = new $className();
			$content = $widget->render($data);
		} else {
			$widget = new ZcWidget();
			$content = $widget->renderFile($route, $data);
		}
	...
	public static function getDbDefaultListener() {
	    return ZcFactory::singleton('ZcDbDefaultListener');
	}
	 ...
	 */
	public static function getCache($bizName, $cacheType = '',  $timestamp = '', $options = null, $cacheClass = '' , $isClean = false) {
		static $cacheInstances = array();
        
		$cacheInstanceKey = $bizName; 
		if( $isClean ){
		    unset($cacheInstances[$cacheInstanceKey]);
		}
	 ...
	 * 总之，需要把Zc的对象池都清空。
	 */
	public static function runMVC($route = '',$error='') {
		...
		if (empty($route)) {
			if (!isset($_GET['route'])) {
				$_GET['route'] = Zc::C(ZcConfigConst::DefaultRoute);
			}
			$route = $_GET['route'];
		}
		...
		return $dispatcher->dispatch($action,$error);
	}

	public static function runCLI($route) {
		global $argv, $argc;

		if (empty($route)) {
			$route = isset($argv[1]) ? $argv[1] : Zc::C(ZcConfigConst::DefaultRoute);
		}
		$action = new ZcAction($route);
		$dispatcher = new ZcDispatcher();
